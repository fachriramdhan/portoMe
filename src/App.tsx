...
export default function App() {
    ...
    <>
      ...
      <div className="relative min-h-screen w-full bg-gray-50 dark:bg-[#050505] text-gray-900 dark:text-white selection:bg-[#b026ff]/30 selection:text-gray-900 dark:selection:text-white flex flex-col lg:flex-row font-sans lg:overflow-hidden transition-colors duration-500">
        ...
        <header className="lg:sticky lg:top-0 lg:flex lg:h-screen lg:w-[45%] xl:w-[40%] lg:flex-col lg:justify-between lg:py-16 lg:pl-24 lg:pr-16 pt-32 pb-12 px-6 sm:px-8 z-10 lg:overflow-hidden no-scrollbar">
          ...
          <div className="mt-12 lg:mt-auto flex items-center gap-3 xl:gap-4 flex-wrap">
  {/* Social Links Group */}
  <div className="flex items-center gap-2.5">
    {[
      ...
      { 
        ...
        icon: <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M.057 24l1.687-6.163c-1.041-1.804-1.588-3.849-1.587-5.946.003-6.556 5.338-11.891 11.893-11.891 3.181.001 6.167 1.24 8.413 3.488 2.245 2.248 3.481 5.236 3.48 8.414-.003 6.557-5.338 11.892-11.893 11.892-1.99-.001-3.951-.5-5.688-1.448l-6.305 1.654zm6.597-3.807c1.676.995 3.276 1.591 5.392 1.592 5.448 0 9.886-4.434 9.889-9.885.002-5.462-4.415-9.89-9.881-9.892-5.452 0-9.887 4.434-9.889 9.884-.001 2.225.651 3.891 1.746 5.634l-.999 3.648 3.742-.981zm11.387-5.464c-.074-.124-.272-.198-.57-.347-.297-.149-1.758-.868-2.031-.967-.272-.099-.47-.149-.669.149-.198.297-.768.967-.941 1.165-.173.198-.347.223-.644.074-.297-.149-1.255-.462-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.297-.347.446-.521.151-.172.2-.296.3-.495.099-.198.05-.372-.025-.521-.075-.148-.669-1.611-.916-2.206-.242-.579-.487-.501-.669-.51l-.57-.01c-.198 0-.52.074-.792.347-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.876 1.213 3.074.149.198 2.095 3.2 5.076 4.487.709.306 1.263.489 1.694.626.712.226 1.36.194 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.695.248-1.29.173-1.414z" /></svg>, 
        color: "hover:text-[#25D366]" 
      },
      { 
        name: "Email", 
        url: "mailto:fachriramdhan04@gmail.com", 
        icon: <span className="material-icons text-[20px]">mail</span>, 
      ...
      <a
        ...
        rel="noreferrer"
        title={social.name}
        className={`p-2.5 rounded-xl bg-gray-200/50 dark:bg-white/5 text-gray-500 dark:text-gray-400 border border-transparent hover:border-gray-300 dark:hover:border-white/10 ${social.color} transition-all duration-300 hover:-translate-y-1 hover:shadow-sm`}
      >
        {social.icon}
      </a>
    ))}
  ...
  <div className="hidden lg:flex items-center gap-2">
    <button
      ...
      className="p-2.5 rounded-xl bg-gray-200/50 dark:bg-white/5 text-gray-600 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-white/10 transition-all duration-300 flex items-center justify-center hover:-translate-y-1 font-bold text-[10px] w-10 h-10 border border-transparent hover:border-[#b026ff]/30"
    >
      {lang === "id" ? "EN" : "ID"}
    </button>
    <button
      onClick={toggleTheme}
      className="p-2.5 rounded-xl bg-gray-200/50 dark:bg-white/5 text-gray-600 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-white/10 transition-all duration-300 flex items-center justify-center hover:-translate-y-1 w-10 h-10 border border-transparent hover:border-[#00f0ff]/30"
    >
      <span className="material-icons text-[18px]">
        ...
        {/* Mobile Bottom Bar */}
        <nav
          className={`lg:hidden fixed bottom-4 left-4 right-4 z-40 bg-white/90 dark:bg-[#111]/90 backdrop-blur-xl border border-gray-200/50 dark:border-white/10 rounded-2xl shadow-lg transition-all duration-500 ${
            isMobileMenuOpen
        ...
        >
          <div className="flex items-center justify-around p-2">
            {allLinks
              .filter((link) =>
                ["about", "skills", "projects"].includes(link.id)
              )
              .map((link) => (
                <a
                  key={link.id}
                  ...
                  className={`flex flex-col items-center justify-center w-[4.5rem] h-14 rounded-xl transition-all duration-300 relative ${
                    activeSection === link.id
                      ? "text-[#b026ff] dark:text-[#00f0ff]"
                ...
                >
                  ...
                  <span
                    className={`material-icons text-[22px] mb-1 transition-transform duration-300 ${
              ...
              <div className="flex-1 flex flex-col">
                ...
                <ul className="flex flex-col gap-2 mb-8">
                  {allLinks
                    .filter(
                      (link) =>
                        !["about", "skills", "projects"].includes(
                          link.id
                        )
                    )
                    .map((link, index) => (
                      ...
                      >
                        <a
                          ...
                          className={`group flex items-center gap-4 p-3 rounded-2xl transition-all duration-300 ${
                            activeSection === link.id
                              ? "bg-gray-50 dark:bg-white/5 text-[#b026ff] dark:text-[#00f0ff]"
                              : "text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-white/5 hover:text-gray-900 dark:hover:text-white"
                          }`}
                        >
                          <span
                            className={`material-icons transition-transform duration-300 group-hover:scale-110 ${
                              activeSection === link.id ? "scale-110" : ""
                            }`}
                          >
                            {link.icon}
        ...
        <div className="lg:w-[55%] xl:w-[60%] relative lg:h-screen flex flex-col w-full">
            ...
            <nav className="pointer-events-auto flex py-2.5 px-5 rounded-full border border-gray-200/50 dark:border-white/10 shadow-sm w-max bg-white/80 dark:bg-[#111]/80 backdrop-blur-xl">
              <ul className="flex flex-wrap gap-x-1 xl:gap-x-2 items-center">
                {allLinks.map((link) => (
                  <li key={link.id}>
                    <a
                      className={`group relative text-[10px] xl:text-xs font-bold uppercase tracking-widest transition-all duration-300 py-2 px-3 rounded-full ${
                        activeSection === link.id
                          ? "text-[#b026ff] dark:text-[#00f0ff] bg-gray-100 dark:bg-white/10"
          ...
          >
            <div className="flex flex-col gap-16 lg:gap-28">
              ...
              <footer className="py-8 text-gray-500 text-sm border-t border-gray-200 dark:border-white/5 mt-1 transition-colors duration-500">
                <p>
                  © {currentYear} Fachri.{" "}
                  {lang === "id"
